...
public class CertificateService implements ICertificateService {
    ...
    public Certificate createNewCertificate(NewCertificateDTO newCertificateDTO) {
        ...
        java.security.cert.Certificate issuerCertificate = keyStoreReader.readCertificate("src/main/resources/static/keystore.jks", keystorePassword, certificateAlias);
        ...
        PrivateKey privateKey = KeyUtils.readPrivateKey("src/main/resources/keys/" + certificateAlias + ".key");
        ...
        aliasMappingService.addSignedCertificate(((X509Certificate) issuerCertificate).getSerialNumber().longValue(),x509Certificate.getSerialNumber().longValue(), subjectAlias);
    ...
    void saveCertificate(X509Certificate cert,Long issuerId,String alias){
        String keystoreLoc = "src/main/resources/static/keystore.jks";
        keyStoreWriter.loadKeyStore(keystoreLoc,keystorePassword.toCharArray());
        keyStoreWriter.write(alias,cert);
        keyStoreWriter.saveKeyStore(keystoreLoc,keystorePassword.toCharArray());
    ...
    void savePrivateKey(PrivateKey privateKey,String alias){
        ...
        try (Writer writer = new FileWriter("src/main/resources/keys/" + alias + ".key")) {
